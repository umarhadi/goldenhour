<script>
  ...
  let totalDuration = 4 * 60 - 8
  let currentTime = 0
  let determinedIndex = 0
  let songTitle = 'Porter Robinson - Musician'
  ...
  onMount(() => {
    window
      .fetch('Porter Robinson - Musician.txt')
      .then((res) => res.text())
      .then((res) => {
        constructLyrics(res)
      })
  ...
  <div class="o-lyrics">
    ...
    {#if isShowCredit}
      <p>
        ...
        <a href="https://github.com/umarhadi/goldenhour/tree/master/musician">@umarhadi</a>
      </p>
    {/if}
  </div>
  <Analyser
    audio="Porter Robinson - Musician.mp3"
    {totalDuration}
    {updateCurrentTime}
    {showCredit} />
</div>
